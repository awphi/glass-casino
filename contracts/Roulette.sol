...
contract Roulette {
  ...
  function random(uint256 mod) private view returns (uint256) {
    return
      uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp))) %
      mod;
  }

  function get_num_bets() public view returns (uint256) {
    return bets.length;
  }
  ...
  function is_red(uint256 roll) private pure returns (bool) {
