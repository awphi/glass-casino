...
contract Roulette {
    ...
    function random(uint256 mod) public view returns (uint256) {
        return uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp))) % mod;
    }
    ...
    function get_winnings(Bet memory bet, uint256 roll) public pure returns (uint256) {
